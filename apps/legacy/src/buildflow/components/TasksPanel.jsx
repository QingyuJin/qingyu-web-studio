...
function TasksPanel({ tasks, toggleTaskComplete, updateTaskReport, deleteTask }) {
  ...
  const workers = Array.from(
    new Map(tasks.map((task) => [task.workerId || "", task.workerName || "未指派"]))
  ).filter(([id]) => id !== "")
  ...
  const filteredTasks = tasks.filter((task) => {
    ...
    const matchKeyword = includesKeyword(
      `${task.projectName} ${task.title} ${task.workerName} ${task.note} ${task.report}`,
      keyword
    )
        ...
        <div className="flex flex-col gap-4 md:flex-row md:items-end md:justify-between">
          <div>
            ...
            <p className="mt-1 text-sm text-slate-500">
              目前顯示 {filteredTasks.length} / {tasks.length} 個任務
            </p>
          </div>
...
function WorkerPanel({ worker, tasks, toggleTaskComplete, updateTaskReport }) {
  ...
  const filteredTasks = tasks.filter((task) => {
    const matchStatus = statusFilter === "全部" || task.status === statusFilter
    const matchKeyword = includesKeyword(
      `${task.projectName} ${task.title} ${task.note} ${task.report}`,
      keyword
    )
        ...
        <Card key={task.id}>
          ...
          <div className="mt-3 flex flex-wrap gap-2">
            <button
              ...
              className={`rounded-xl px-4 py-3 text-sm font-black ${task.status === "已完成" ? "bg-slate-200 text-slate-700" : "bg-slate-950 text-white"}`}
            >
