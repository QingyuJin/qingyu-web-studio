...
function VendorsPanel({ vendors, addVendor, editVendor, deleteVendor }) {
  ...
  const filteredVendors = vendors.filter((vendor) =>
    includesKeyword(
      `${vendor.name} ${vendor.trade} ${vendor.phone} ${vendor.area} ${vendor.note}`,
      keyword
    )
  )
        ...
        <div className="flex flex-col gap-4 md:flex-row md:items-end md:justify-between">
          <div>
            ...
            <p className="mt-1 text-sm text-slate-500">
              目前顯示 {filteredVendors.length} / {vendors.length} 筆廠商資料
            </p>
          </div>
          <input
            value={keyword}
            onChange={(event) => setKeyword(event.target.value)}
            placeholder="搜尋廠商 / 工種 / 地區"
            className="rounded-xl border border-slate-200 px-4 py-3 text-sm outline-none focus:border-slate-500 md:w-[260px]"
          />
        </div>
