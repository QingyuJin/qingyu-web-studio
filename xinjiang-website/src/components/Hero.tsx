...
export function Hero() {
  return (
    <section id="top" className="relative overflow-hidden pt-24 md:pt-28">
      <div className="absolute inset-0 -z-10 bg-[radial-gradient(circle_at_18%_12%,rgba(231,190,111,0.24),transparent_32rem),radial-gradient(circle_at_88%_18%,rgba(185,94,47,0.13),transparent_30rem),linear-gradient(180deg,#fbf4e7,#efe1cc)]" />
      <div className="mx-auto grid min-h-[calc(100svh-7rem)] max-w-7xl gap-8 px-4 py-12 md:px-8 lg:grid-cols-[0.92fr_1.08fr] lg:items-center">
        <div>
          ...
          <p className="mt-6 max-w-2xl text-base font-semibold leading-8 text-[#6e604f] md:text-lg">
            40 年老師父經驗，泥作、磁磚、洗石子、牆地面、水泥、油漆、拆除整修，到場評估後實在報價。
          </p>
          ...
          <div className="mt-8 grid gap-3 sm:grid-cols-3">
            {proofPoints.map(([value, label]) => (
              <div key={value} className="rounded-[1.1rem] border border-[#e1cdae] bg-white/58 p-4 shadow-sm">
                <p className="text-2xl font-black tracking-[-0.04em] text-[#b95e2f]">{value}</p>
                <p className="mt-1 text-xs font-black text-[#766858]">{label}</p>
              </div>
            ))}
          </div>
        </div>
        ...
        <div className="relative">
          ...
          <div className="relative overflow-hidden rounded-[2rem] border border-[#e3d3bc] bg-white/64 p-3 shadow-[0_28px_90px_rgba(58,45,31,0.14)]">
            <img src="/project-photos/pro360-1.jpg" alt="泥作牆面施工現場" className="aspect-[5/4] w-full rounded-[1.55rem] object-cover object-center md:aspect-[5/3]" />
            <div className="absolute bottom-6 left-6 right-6 rounded-[1.35rem] border border-white/60 bg-[#fbf4e7]/82 p-5 backdrop-blur-xl">
              ...
              <p className="mt-2 text-lg font-black text-[#2b2118]">老師父看工法，年輕學徒協助溝通與整理。</p>
            </div>
          </div>
        </div>
      </div>
